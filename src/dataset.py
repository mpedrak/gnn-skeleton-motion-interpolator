...
from .utils.bvh import parse_bvh_file, build_edge_index_from_parents
...
class GraphSkeletonDataset(Dataset):
    def __init__(self, root_dir, context_len_pre, context_len_post, target_len, step):
        ...
        for fname in self.files:
            ...
            if os.path.exists(cache_path):
                ...
                data = {
                    'root_pos': torch.tensor(root_pos, dtype=torch.float),
            ...
            all_root_positions.append(data['root_pos']) 

            frames = data['rot_6d'].shape[0]
            used_frames = context_len_pre + context_len_post + target_len
            for start in range(0, frames - used_frames, step):
        ...
        concat_root = torch.cat(all_root_positions, dim=0) # [F, 3]
        self.root_mean = concat_root.mean(dim=0)            
        self.root_std = concat_root.std(dim=0)
    ...
    def __getitem__(self, idx):
        ...
        first_part = data['root_pos'][start : start + self.context_len_pre] 
        second_part = data['root_pos'][post_ctx_start : post_ctx_start + self.context_len_post]
        root_ctx_raw = torch.cat([first_part, second_part], dim=0)

        root_tgt_raw = data['root_pos'][start + self.context_len_pre : post_ctx_start]  
